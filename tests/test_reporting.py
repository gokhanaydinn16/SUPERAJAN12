...
def _build_scan_result(market_id: str, score: float, *, category: str = "politics") -> ScanResult:
    return ScanResult(
...
def test_top_scored_markets_defaults_to_latest_scan(tmp_path) -> None:
    ...
    store.save_scan(_build_scan_result("older-market", 999.0, category="macro"))
    store.save_scan(_build_scan_result("latest-market", 100.0, category="crypto"))
    ...
    assert latest_only[0]["category"] == "crypto"
...
def test_category_summary_groups_latest_scan(tmp_path) -> None:
    path = tmp_path / "superajan12.sqlite3"
    store = SQLiteStore(path)
    reporter = Reporter(path)

    store.save_scan(_build_scan_result("older-market", 200.0, category="macro"))
    store.save_scan(_build_scan_result("latest-market", 100.0, category="crypto"))

    rows = reporter.category_summary()

    assert len(rows) == 1
    assert rows[0]["category"] == "crypto"
    assert rows[0]["market_count"] == 1
    assert rows[0]["approve_count"] == 1
